...
const changeLanguage = function (language) {
  ...
  for (const item of languageNavbar.querySelectorAll(
  ...
  if (newActiveElem.classList.contains('dropdown-item')) {
    const parent = newActiveElem.parentElement.parentElement.parentElement.getElementsByClassName(
      'dropdown-toggle'
    )[0];
    if (parent) {
...
const languageNavbar = document.getElementById('language-navbar');
const languageNavbarItems = languageNavbar.querySelectorAll(
...
try {
  ...
  if (
    ...
    browsers.includes(detectedBrowser.name)
  ) {
    userBrowser = detectedBrowser.name;
    if (
      detectedBrowser.os &&
      detectedBrowser.os.toLowerCase().includes('windows')
    ) {
      userOS = 'windows';
    }
  }
...
for (const b of browsers) {
  ...
  for (const basicAuthExample of basicAuthExamples) {
    basicAuthExample.addEventListener('click', function (e) {
      e.preventDefault();
      const browser = [...e.target.classList].filter((c) =>
        c.startsWith('to-')
      );
      if (browser.length) {
